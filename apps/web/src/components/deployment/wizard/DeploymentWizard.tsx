...
const WIZARD_STEPS: WizardStep[] = [
  { id: 1, label: "Configuration", description: "Provider, extensions & YAML" },
  { id: 2, label: "Region", description: "Select deployment region" },
...
function validateStep(step: number, config: DeploymentConfig): string | null {
  if (step === 1) {
    ...
    // Extensions are optional — no validation required
    if (!config.yamlConfig.trim()) return "YAML configuration is required";
    return null;
  }
  if (step === 2) {
