...
export interface SindriYamlEditorProps {
  /** Controlled mode: parent owns the content. Takes priority over initialValue. */
  value?: string;
  /** Uncontrolled mode: seeds internal state once on mount. */
  initialValue?: string;
...
export function SindriYamlEditor({
  ...
  const [internalYaml, setInternalYaml] = useState(initialValue);

  // In controlled mode the parent's value is the source of truth;
  // in uncontrolled mode we manage state internally.
  const yaml = controlledValue !== undefined ? controlledValue : internalYaml;
  ...
  const handleChange = useCallback(
    (v: string) => {
      if (controlledValue === undefined) setInternalYaml(v);
      onChange?.(v);
    },
